CREATE OR REPLACE PACKAGE BODY BANINST1.pk_CargaBChile IS
/******************************************************************************
...
Modificación :    md-04 
Autor  :   Roman ruiz
Acción :   Cambio de procedimiento de p_pagardocu
Fecha  :   07-sep-2016
...
******************************************************************************/
 ...
 cgsCodCarga          constant varchar2(3) := 'BCH';   --md-04
...
BEGIN
 ...
 IF UTL_RAW.COMPARE(vrTmp, vrBOMUTF8) = 0 THEN
    ...
    DBMS_LOB.COPY( vrBlobUTF
                  ,vrArchivoBLOB
                  ,DBMS_LOB.GETLENGTH(vrArchivoBLOB)-3
                  ,1
                  ,4 );
 ...
 ELSE
    --Si no encontró el BOM asumo que es la codificación estandar
    --ISO-8859-1
 ...
 WHILE vni <= vnTamanoCLOB LOOP
  ...
  IF vsLinea IS NOT NULL THEN
   ...
   IF vnNumLineas = 1 THEN
      ...
      -- la guardo para procesarla al final!!!
      vsLineaEnc := vsLinea;
      ...
      --Lo bueno de oracle 11g es que ya existe el continue
      --jojojojojo
      CONTINUE;
