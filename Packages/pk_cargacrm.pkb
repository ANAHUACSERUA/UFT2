DROP PACKAGE BODY BANINST1.PK_CARGACRM;

CREATE OR REPLACE PACKAGE BODY BANINST1.pk_CargaCRM IS
/******************************************************************************
...
BEGIN
    ...
    WHILE vni <= vnTamano LOOP
        ...
        IF vsLinea IS NOT NULL THEN
            ...
            CASE vsStatus
                WHEN 'A' THEN
                    p_ImprimeLineaResultado('Registro Correcto: '
                        || vsMensajeProc);
                WHEN 'W' THEN
                    p_ImprimeLineaResultado('Advertencia: '
                        || vsMensajeProc);
                WHEN 'R' THEN
                    p_ImprimeLineaResultado('Registro no procesado: '
                        || vsMensajeProc);
                WHEN 'E' THEN
                    p_ImprimeLineaResultado('Error: '
                        || vsMensajeProc);
                ELSE
