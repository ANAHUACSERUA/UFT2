CREATE OR REPLACE PACKAGE BODY BANINST1.pk_AdmCarga IS
/******************************************************************************
PAQUETE:            BANINST1.pk_AdmCarga
...
****************************************************
mod       :         md-02
objetivo  :         se QUITA subir archivo CAE-postulantes Y CAE-Superior
                    y se agrega carga de nombres legales.
autor     :         Roman Ruiz
fecha     :         19-jun-2014
...
PROCEDURE p_Main(
    ...
    CURSOR cuApps IS
        SELECT
            'uft_ccrm'            AS IdApp
            ,'CARGA CRM'          AS DescApp
        FROM DUAL
        UNION ALL
        SELECT
            'uft_cpsu'                       AS IdApp
            ,'CARGA RESULTADOS PSU'          AS DescApp
        FROM DUAL
        UNION ALL
        SELECT
            'uft_cccp'              AS IdApp
            ,'CARGA COLEGIOS DE PROCEDENCIA' AS DescApp
        FROM DUAL
        UNION ALL
        SELECT
            'uft_cpe'              AS IdApp
            ,'CARGA POSTULACIONES EFECTIVAS' AS DescApp
        FROM DUAL
        union all
        SELECT
            'uft_cnlg'              AS IdApp
            ,'CARGA NOMBRE LEGAL' AS DescApp
        FROM DUAL
        ORDER BY DescApp;
    ...
    /* md-02 incio    
     -- md-01 inicio
        ...
        SELECT
            'uft_caep'              AS IdApp
            ,'CARGA CAE POSTULANTES' AS DescApp
        FROM DUAL
        UNION ALL
        SELECT
            'uft_caes'              AS IdApp
            ,'CARGA CAE SUPERIOR' AS DescApp
        FROM DUAL      -- md-01 fin
        ORDER BY DescApp;
    md-02 fin   */
 ...
 HTP.P('
  ...
  ');
      ...
      htp.p('
        ...
        <h2>
            Carga de archivos de admisi&oacute;n.
        </h2>
        <hr/>
        <br/>
