...
modiify
se agrega orden alfabetico al parametro psprog1
y tambien va el cambio de PWRBFINH
04-nov-2014   vic....

 --md-01
                 18-nov-2014
                 roman ruiz
                 * se agrega parametros y procedure para
                 archivo balance final

  --md-02       23-abr-2015
                roman ruiz
                se agrea parametro Si / NO

                -- md-03 vdj 20150429
                se agrega parametro para PWRMARA

  --Modificación: 21/07/2015
                  MAO
                  * Se agrega select para el parametro psPerio en la función FWASLCT

   --md-04      24-nov-2015
                 roman ruiz
               se agrega el parametro de anio del acutal hasta el 1999 de mayor a menor

   --md-05      30-nov-2015
                 marcela altamirano
               se agreaga en nivel para el reporte pwrcptd
...
*/
  ...
  --Retorna el objeto select con el catalogo de periodos Matricula
  PROCEDURE PWTERMAT;
...
--retorna el objeto select con los Código Cohorte
  ...
  PROCEDURE PWCDETC(psTypeInd VARCHAR2 DEFAULT NULL,psDcatCod VARCHAR2 DEFAULT NULL );
...
--retorna el objeto select con la consulta del atributo de admisiones
  ...
  PROCEDURE PWINTER;
--VDJ Procedimiento para seleccionar si se quiere traer los intereses de los documentos

PROCEDURE PWSTCNT;
--vdelacruz 20150910 Procedimiento para seleccionar status del contrato
...
--md-01 start
--retorna el objeto select con los archivos de balance final
  PROCEDURE PWRBAFL;

--md-01 end

--md-02 start
--retorna el objeto select con valores de Si/No
  PROCEDURE PWRSINO;

--md-02 end


  procedure PWRANIO;       --md-04
  ...
  BEGIN
      ...
      -- 21/07/2015 MAO
      ELSIF psParametro IN ('psPerio','psPeriM','psTermC','psTerm','psPeri1', 'psPeriA', 'psPerMa') THEN
            OPEN cuQry FOR
                 SELECT STVTERM_CODE                   AS cCode,
                        STVTERM_CODE||' - '||REPLACE(STVTERM_DESC,'''',' ') AS cDesc,
                        NULL AS cDsc1,
                        NULL AS cDsc2,
                        NULL AS cDsc3,
                        NULL AS cDsc4
                   FROM STVTERM
                  WHERE STVTERM_TRMT_CODE IS NOT NULL
                    AND STVTERM_ACYR_CODE  = psFiltro1
                  ORDER BY STVTERM_CODE DESC;
  ...
  BEGIN
      ...
      ELSIF vgsParametro = 'psPerMa' THEN--hfn periodo matrícula
            PWTERMAT;
      ELSIF vgsParametro = 'psPerIn' THEN-- md-03 vdj 20150429
           PWCTERM;
      ...
      ELSIF vgsParametro = 'psDetPS'  THEN
            PWCDETC('P','SER');
      ...
      ELSIF vgsParametro = 'psNacio' THEN
            PWCCITZ;
      ELSIF vgsParametro = 'psEmail' THEN
            PWREMAL;
      ...
      ELSIF vgsParametro = 'psNatn' THEN
            PWCNATN;

      ELSIF vgsParametro = 'psSstyp' THEN     --verificar si no es 'psSstst'
            PWCSTYP;
     ...
     ELSIF psParametro = 'psAño'  THEN
      ...
      ELSIF psParametro = 'psAnio'  THEN
            PWCMATRA;

      --md-04 stat
      ELSIF psParametro = 'psAnioI'  THEN
            PWRANIO;
     ...
     ELSIF psParametro = 'psCatpr'  THEN
      ...
      -- md-01 start   --archivos de balance final
      ELSIF psParametro ='psBaFl' THEN
              PWRBAFL;
      -- md-01 end
      --md-02 stat
      ELSIF psParametro = 'psSiNo' then
            PWRSINO;
      --md-02 end
        --vdelacruz 20150910
ELSIF psParametro = 'psStCnt' then
     PWSTCNT;
...
--Procedimiento hora
  ...
  BEGIN
      ...
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWMATCO') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRBFIN', 'PWRBFINH') THEN  ---md vic... 31.10.2014
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRADCFC') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRCENV') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRMINO') THEN
            concatenar('vsValue1','psNivl1');
      ...
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRSACD') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRMASI') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWMATCO') THEN
            concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWLIBVE') THEN
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRACS') THEN   ---M.D.  vic 2-dic-2014 rep (pwracs)
            concatenar('vsValue1','psNivl1');
      ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRMARA') THEN  -- md-03 vdj 20150429
            concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRCAOD') THEN  -- md-03 vdj 20150429
        concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRPGCP') THEN  -- md-03 vdj 20150429
        concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRCAPP') THEN  -- md-03 vdj 20150429
        concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRRCNU') THEN  -- md-03 vdj 20150429
        concatenar('vsValue1','psNivl1');
        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRCPTD') THEN  -- md-05
        concatenar('vsValue1','psNivl1');

        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRPGC2') THEN  -- md-05
        concatenar('vsValue1','psNivl1');
         ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRPGC3') THEN  -- md-05
        concatenar('vsValue1','psNivl1');

        ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRGRAV') THEN  -- md-05
        concatenar('vsValue1','psNivl1');
         ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRGRAN') THEN  -- md-05
        concatenar('vsValue1','psNivl1');
         ELSIF vgsParametro = 'psNivl1' AND psReporte IN ('PWRRET2') THEN  -- md-05
        concatenar('vsValue1','psNivl1');
     ...
     CURSOR cuQry IS
        SELECT STVTERM_CODE cCode,
         ...
         WHERE EXISTS(SELECT 1
                        FROM TWBCNTR
                       WHERE TWBCNTR_TERM_CODE=STVTERM_CODE)
        ORDER BY STVTERM_CODE DESC;
  BEGIN
     FOR regQry IN cuQry LOOP
...
ORDER BY STVCHRT_CODE;
  ...
  PROCEDURE PWCDETC(psTypeInd VARCHAR2 DEFAULT NULL
                   ,psDcatCod VARCHAR2 DEFAULT NULL
                   ) IS
  CURSOR cuQry IS
         SELECT TBBDETC_DETAIL_CODE cCode,
          ...
          WHERE(TBBDETC_TYPE_IND  = psTypeInd OR psTypeInd IS NULL)
            AND(TBBDETC_DCAT_CODE = psDcatCod OR psDcatCod IS NULL)
  ...
  PROCEDURE PWREMAL IS

  CURSOR cuQry IS
         SELECT GTVEMAL_CODE cCode,
                GTVEMAL_DESC cDesc
           FROM GTVEMAL
          WHERE GTVEMAL_CODE     IN ('UFT','PERS')
...
--retorna el objeto select con la consulta del status stvnatn
  PROCEDURE PWCNATN IS

  CURSOR cuQry IS
         SELECT STVNATN_CODE cCode,
                STVNATN_NATION cDesc
           FROM STVNATN
          ORDER BY STVNATN_NATION;
...
PROCEDURE PWCTERP IS
  ...
  PROCEDURE PWINTER IS

  BEGIN

          htp.p('<option value="SI">SI</option>');
          htp.p('<option value="NO">NO</option>');

      END PWINTER;

 --md-01 start
 PROCEDURE PWRBAFL IS

  CURSOR cuQry IS
        SELECT   GWBGEFA_FILE_NUM cCode, SUBSTR(GWBGEFA_FILE_NAME,1, 25) cDesc
        FROM  GWBGEFA
        WHERE GWBGEFA_PROCESS = 'FOBF'
        ORDER BY  GWBGEFA_FILE_NUM;
  ...
  --md-01 end
  --md-02 start
 PROCEDURE PWRSINO IS

  CURSOR cuQry IS
       select 1 cCode,  'Si' cDesc from dual
       union
       select 0 cCode,  'No' cDesc from dual;
  ...
  --md-04 start
   ...
   procedure PWRANIO  IS

    CURSOR cuQry IS
         SELECT DISTINCT (SUBSTR (STVTERM_CODE, 1, 4)) cCode
         FROM STVTERM
         where SUBSTR (STVTERM_CODE, 1, 4) >= 1999
         and  SUBSTR (STVTERM_CODE, 1, 4) <> 9999
         ORDER BY SUBSTR (STVTERM_CODE, 1, 4) desc;
...
-- vdelacruz 20150910
  CURSOR cuQry IS
       select 'A' cCode, 'Activo' cDesc from dual
       union
        select 'X' cCode, 'Sustituido' cDesc from dual
        union
        select 'C' cCode, 'Cancelado' cDesc from dual;
