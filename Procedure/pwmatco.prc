...
******************************************************************************/
   ...
   vsNivel     VARCHAR2(25)                   := NULL;
     csSlh        CONSTANT VARCHAR2(1)    := '/';
--   vnTest       VARCHAR2(10);
CURSOR cuReporte_Cambio (
   vsNivel in stvlevl.stvlevl_code%TYPE,
   vsProg in smrprle.smrprle_program_desc%TYPE,
...
--       , f_get_status(twbcntr_pidm, sgbstdn_stst_code)                                                  actualizacion_estatus
       , F_GET_PROGRAMA_ANT(TWBCNTR_PIDM, TWBCNTR_TERM_CODE, SGBSTDN_PROGRAM_1)                       program_anterior
       , F_GET_PROGRAMA_vig(TWBCNTR_PIDM, TWBCNTR_TERM_CODE)                                            program_vigente
...
where (programa = vsProg or vsProg is null)
  and (instr(csSlh||vsNivel , csSlh||nivel||csSlh) > 0 or   vsNivel = csSlh  OR  vsNivel is null)
--and (nivel= substr(vsNivel,1,2) or substr(vsNivel,1,2) is null)
  -- and (instr(csSlh||substr(vsNivel,1,2) , csSlh||nivel||substr(vsNivel,1,2)) > 0 or   substr(vsNivel,1,2) = csSlh  OR  substr(vsNivel,1,2) is null)
...
BEGIN
    ...
    vsProg  := pk_ObjHtml.getValueCookie ('psProg1');
...
--   tabColumna (16) := 'Actualizaci?n Status Alumno ';
   ...
   tabColumna (19) := 'Programa Vigente';
   tabColumna (20) := 'Programa Contrato';
   tabColumna (21) := 'Descripci&oacute;n Programa';
...
--   tabColumna (26) := 'E-mail Personal';
      ...
      FOR regRep IN cuReporte_Cambio(vsNivel, vsProg, vsSsts, vsType, vsYear, vdIni, vdFin, vsCont) LOOP
...
--                      and y.sgbstdn_term_code_eff<= regRep.periodo_cntr);
          ...
          IF vsStatusCntr <> '' THEN
            ...
            <td valign="top">'||regRep.program_vigente||'</td>
