CREATE OR REPLACE PACKAGE BODY BANINST1.pk_Salida_Rep_Moro_4 IS
/******************************************************************************
...
******************************************************************************/
    ...
    cgsCodApp       CONSTANT VARCHAR2(4) := 'RMM4';                       --md-v
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                  'pk_Salida_Rep_Moro_4.p_ObtDetOper', NULL);
...
PROCEDURE p_Genera( pnNumArch   OUT        PLS_INTEGER
    ...
    --vrDetOper            t_DetOper;
    --El hash table en si
    --vhOpers               pk_Salida_Rep_Moro_4.t_HshOper;
  ...
  --Cursor del reporte moro 4   --md-v
/*    CURSOR cuDatos IS
SELECT
...
WHERE DOMV1.TWRDOMV_DOCU_SEQ_NUM = TWBDOCU_SEQ_NUM
  ...
  and TWBDOCU_STATUS_IND = 'PA'
  AND TWBDOCU_TERM_CODE LIKE to_char( psAnio ||'%' )
  AND TWBCNTR_ORI_PROGRAM =TWRPRCC_PROGRAM
  AND TWBCNTR_NUM = TWBDOCU_CNTR_NUM
  and TWBCNTR_PIDM = spriden_PIDM
...
BEGIN
    ...
    if psAnio = 'xxxx' then
       OPEN cuDatosPromNull;
          FETCH cuDatosPromNull BULK COLLECT INTO vtDatosMov;
       CLOSE cuDatosPromNull;
    else
       OPEN cuDatos;
...
BEGIN
    ...
    --Genero archivo CAE.
    p_Genera(vnNumArch, vnNumRegs, vdFecha, pk_login.vgsUSR, psAnio );
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                   'pk_Salida_Rep_Moro_4.p_ObtArchivoNuevo', NULL);
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                 'pk_Salida_Rep_Moro_4.p_ReimprimeArchivo', NULL);
...
BEGIN
    ...
    HTP.P('<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
          <HTML>
              <HEAD>
               <TITLE>Generaci&oacute;n de Archivo Morosidad 4</TITLE>'  );
    ...
    <BODY>
        ...
        <h2>
          Reporte 4 Morosidad = Todos los Pagos Recibidos y Promoción al Año Seleccionado.
        </h2>
        <h4>
         <p>Filtros <br/>
         Promoción = Año seleccionado  <br/>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%" bgcolor="#efefef" >
            <form name="frmDatos" method="post">
                ...
                <tr>
                    ...
                    <td class="etiqueta">
                      Promoción :&nbsp;&nbsp;
                      <select name="psAnio"  style="width:200px" >
    ...
    HTP.P('
        ...
        function BuscarArchivos(){
            ...
            ExecAjax("pk_Salida_Rep_Moro_4.p_JSONListaArchivos",prms,CargaComboArchivos);
    ...
    //funcion para el boton recuperar
        function Recuperar(){
            ...
            frmParams.action = "pk_Salida_Rep_Moro_4.p_ReimprimeArchivo";
...
//            }
        ...
        function Generar(){
           // if(!ValidaNuevo()) return;
            ...
            //indico el destino de la generacion de archivos
            frmParams.action = "pk_Salida_Rep_Moro_4.p_ObtArchivoNuevo";
            ...
            frmParams.psAnio.disabled = false;

            frmParams.psFecha.value = frmDatos.txtFecha.value;
            frmParams.psAnio.value = frmDatos.psAnio.value;
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                  'pk_Salida_Rep_Moro_4.p_Main', NULL);
