CREATE OR REPLACE PACKAGE BODY BANINST1.pk_Salida_Rep_Moro_1 IS
/******************************************************************************
PAQUETE:            BANINST1.pk_Salida_Rep_Moro_1
...
******************************************************************************/
    ...
    cgsCodApp       CONSTANT VARCHAR2(4) := 'RMM1';                       --md-v
    ...
    csSERV          constant varchar2(4) := 'SERV';     --doc origine servicio = curso
...
OBJETIVO:          detalle de operacion se deja para no quitar el spec 
PARAMETROS:
  ...
  EXCEPTION
    WHEN OTHERS THEN
        ...
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                  'pk_Salida_Rep_Moro_1.p_ObtDetOper', NULL);
...
PROCEDURE p_Genera( pnNumArch   OUT        PLS_INTEGER
    ...
    vrDetOper            t_DetOper;
    --El hash table en si
    vhOpers               pk_Salida_Rep_Moro_1.t_HshOper;
  ...
  --Cursor del reporte moro 1   --md-v
    CURSOR cuDatos IS
                 ...
                 WHERE DOMV1.TWRDOMV_DOCU_SEQ_NUM = TWBDOCU_SEQ_NUM
                     ...
                     AND DOMV1.TWRDOMV_STATUS_IND IN ('AC', 'CE', 'JC', 'JG', 'PR', 'RE', 'TR') -- todos los activos
...
BEGIN
    ...
    --Abro el cursor para registros del cae
    OPEN cuDatos;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
            'pk_Salida_Rep_Moro_1.p_ObtArchivoNuevo', NULL);
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
            'pk_Salida_Rep_Moro_1.p_ReimprimeArchivo', NULL);
...
<HTML>
    <HEAD>
        <TITLE>Generaci&oacute;n de Archivo Morosidad 1</TITLE>'
    );
    ...
    <BODY>
        ...
        <h2>
          Reporte 1 Morosidad = todas las cuotas por cobrar (Vigentes), todas las promociones
       </h2>
        <h4>
         <p>Filtros <br/>
        ...
        Status  = todas las que tengan (AC;CE;JC;JG;PR;TR ETC.)<br/>
        </p>
        </h4>
        <hr/>
        <br/>
    ...
    HTP.P('
        ...
        function BuscarArchivos(){
            ...
            ExecAjax("pk_Salida_Rep_Moro_1.p_JSONListaArchivos",prms,CargaComboArchivos);
    ...
    //funcion para el boton recuperar
        function Recuperar(){
            ...
            frmParams.action = "pk_Salida_Rep_Moro_1.p_ReimprimeArchivo";
...
//            }
        ...
        function Generar(){
           // if(!ValidaNuevo()) return;
            ...
            //indico el destino de la generacion de archivos
            frmParams.action = "pk_Salida_Rep_Moro_1.p_ObtArchivoNuevo";
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
            'pk_Salida_Rep_Moro_1.p_Main', NULL);
