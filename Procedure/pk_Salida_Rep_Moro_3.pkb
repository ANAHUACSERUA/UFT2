CREATE OR REPLACE PACKAGE BODY BANINST1.pk_Salida_Rep_Moro_3 IS
/******************************************************************************
PAQUETE:            BANINST1.pk_Salida_Rep_Moro_3
...
AUTOR:              Roman Ruiz
FECHA:              06-mar-2014
--BANINST1.p_ObtDetOper
-----------------------------
cambio              md-01
objetivo            se agrea parametro de  fecha incial
autor               roman ruiz
fecha               19 ene 2015
-----------------------------
cambio              md-02
...
------------------------------------
modificacion        md-03
objetivo            cambio de campo status de domv a docu
autor               roman ruiz/marcela altamirano
fecha               24/Ago/2015
------------------------------------
modificacion      md-04
objetivo            solo se reportaran documentos pagados en el periodo solicitado
autor               roman ruiz
fecha               22/Dic/2015
...
******************************************************************************/
    ...
    cgsCodApp       CONSTANT VARCHAR2(4) := 'RMM3';                       --md-v
    ...
    csCH            constant varchar2(2) := 'CH';     --carga historica
    cschz           constant varchar2(3) := 'CHZ';     --carga historica rezagado   --md-05
    csEG            constant varchar2(2) := 'EG';
    csEGZ           constant varchar2(3) := 'EGZ';    --status eggresado rezagado   --md-05
    csEGS           constant varchar2(3) := 'EGS';    --Status unico Egresado.
...
OBJETIVO:          se quita todo el desarrollo y se deja solo la definición
                   esto por si en un futuro hay que hacer calculos y no vuelva
                   a poner en spec y body.
PARAMETROS:
prDet                anio de extracción.
******************************************************************************/
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                  'pk_Salida_Rep_Moro_3.p_ObtDetOper', NULL);
...
PROCEDURE p_Genera(  pnNumArch   OUT        PLS_INTEGER
                    ...
                    ,pdFecha1   in date default sysdate ) IS
    ...
    vrDetOper            t_DetOper;
    --El hash table en si
    vhOpers               pk_Salida_Rep_Moro_3.t_HshOper;
    ...
    CURSOR cuDatos IS
           SELECT
              ...
              -- ,DOMV1.TWRDOMV_STATUS_IND                  AS Status
               ,TWBDOCU_STATUS_IND                        AS Status                            -- md-03
               ...
               ,(SELECT STVLEVL_DESC FROM STVLEVL WHERE STVLEVL_CODE = SMRPRLE_LEVL_CODE) as Nivel        --md-01
           FROM
               TWBDOCU
               ,TWRDOMV DOMV1
               ,TWRPRCC
           ...
           WHERE DOMV1.TWRDOMV_DOCU_SEQ_NUM = TWBDOCU_SEQ_NUM
               AND DOMV1.TWRDOMV_MOVE_NUM = ( SELECT MAX(DOMV2.TWRDOMV_MOVE_NUM)
                                               ...
                                               WHERE DOMV2.TWRDOMV_DOCU_SEQ_NUM = DOMV1.TWRDOMV_DOCU_SEQ_NUM
                                                -- AND TRUNC(DOMV2.TWRDOMV_MOVE_DATE) >= TRUNC(TO_DATE('01/01/'||to_char(sysdate,'YYYY'),'DD/MM/YYYY')) --fecha inicio   md-01
                                                 AND TRUNC(DOMV2.TWRDOMV_MOVE_DATE) >= pdFecha1 --fecha inicio   md-01
                                                 AND TRUNC(DOMV2.TWRDOMV_MOVE_DATE) <=  pdFecha
                                                 AND TWRDOMV_STATUS_IND = 'PA'                               --md-04
                                             )
               and TWBDOCU_STATUS_IND = 'PA'
          ...
          union                                                        --md-01 start
             ...
             AND DOMV1.TWRDOMV_MOVE_NUM = ( SELECT MAX(DOMV2.TWRDOMV_MOVE_NUM)
                                                                       FROM TWRDOMV DOMV2
                                                                      WHERE DOMV2.TWRDOMV_DOCU_SEQ_NUM = DOMV1.TWRDOMV_DOCU_SEQ_NUM
                                                                      AND TRUNC(DOMV2.TWRDOMV_MOVE_DATE) >= pdFecha1
                                                                      AND TRUNC(DOMV2.TWRDOMV_MOVE_DATE) <=  pdFecha
                                                                      AND TWRDOMV_STATUS_IND = 'PA'                       --md-04
                                                                      )
...
BEGIN
    ...
    --Abro el cursor para registros de movimientos
    OPEN cuDatos;
...
******************************************************************************/
PROCEDURE p_InsertaReg( pnNumArch            PLS_INTEGER
                        ,pnNumReg            PLS_INTEGER
                        ,psLinea            VARCHAR2
                        ,psUser             VARCHAR2   ) IS
...
BEGIN
    ...
    vdFecha1 := to_date(psFecha1, 'DD/MM/YYYY');
    ...
    p_Genera(vnNumArch, vnNumRegs, vdFecha, pk_login.vgsUSR, vdFecha1);
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
                                  'pk_Salida_Rep_Moro_3.p_ObtArchivoNuevo', NULL);
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
            'pk_Salida_Rep_Moro_3.p_ReimprimeArchivo', NULL);
...
BEGIN
    ...
    HTP.P(
           ...
           <HTML>
               <HEAD>
                  <TITLE>Generaci&oacute;n de Archivo Morosidad 3</TITLE>'
          );
    ...
    <BODY>
        ...
        <h2>
          Reporte 3 Morosidad = todos los pagos recibidos, de todas las promociones
        </h2>
        <h4>
         <p>Filtros <br/>
         Fecha Inicial = Fecha seleccionada <br/>
         Fecha Final  = Fecha de corte seleccionada en esta página<br/>
         Medios Pagos = todos<br/>
         Status  = todos los PAGADOS<br/>
        </p>
        </h4>
        <hr/>
        <br/>
        <table border="0" cellpadding="0" cellspacing="0" width="100%" bgcolor="#efefef" >
            <form name="frmDatos" method="post">
                ...
                <tr>
                    ...
                    <td class="etiqueta" >
                        Fecha Inicial :
                        <input name="txtFecha1" type="text" />

                        <img src="/imagenes/calendario_sin.gif" id="imgCal" onclick="show_calendar('|| chr(39)||'frmDatos.txtFecha1'|| chr(39)||');" />
        ...
        <form name="frmParams" method="post">
            ...
            <input type="hidden" name="psFecha1">
        </form>
    </BODY>'
    );
    ...
    HTP.P('
        ...
        function BuscarArchivos(){
            ...
            ExecAjax("pk_Salida_Rep_Moro_3.p_JSONListaArchivos",prms,CargaComboArchivos);
    ...
    //funcion para el boton recuperar
        function Recuperar(){
            ...
            frmParams.action = "pk_Salida_Rep_Moro_3.p_ReimprimeArchivo";
            ...
            // md-02 start
            frmParams.psFecha1.disabled = true;
            //md-02 end
...
//            }
        ...
        function ValidaNuevo(){
            ...
            if(frmDatos.txtFecha1.value==""){
                alert("No se especifico una Fecha de Inicio Valida.");
                frmDatos.txtFecha1.focus();
                return false;
            }
        ...
        function Generar(){
            ...
            //indico el destino de la generacion de archivos
            frmParams.action = "pk_Salida_Rep_Moro_3.p_ObtArchivoNuevo";
            ...
            frmParams.psFecha1.disabled = false;

            frmParams.psFecha.value = frmDatos.txtFecha.value;
            frmParams.psFecha1.value = frmDatos.txtFecha1.value;
...
EXCEPTION
    WHEN OTHERS THEN
        --pantallazo de error.
        pk_ObjHTML.p_ReporteError(sqlcode,replace(sqlerrm,'"','\"'),
            'pk_Salida_Rep_Moro_3.p_Main', NULL);
